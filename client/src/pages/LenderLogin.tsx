...
export default function LenderLogin() {
  ...
  const loginMutation = useMutation({
    mutationFn: async (data: LoginForm) => {
      const res = await apiRequest("POST", "/api/lenders/login", data);
      if (!res.ok) {
      ...
      <div className="min-h-[calc(100vh-16rem)] py-16 bg-background">
        <div className="max-w-md mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8 text-center">
            <h1 className="text-3xl font-bold text-primary mb-4">Lender Login</h1>
            <div className="h-1 w-24 bg-accent mx-auto mb-4"></div>
            <p className="text-muted-foreground">
              Sign in to access your lender portal
            </p>
          </div>
