...
class SignupFollowupService {
  ...
  async checkAndSendDay7Emails() {
    try {
      // DISABLED: Now handled by Zoho CRM workflow "2 of 5 First Deal Check-in" (fires at 7 days).
  ...
  async checkAndSendFollowups() {
    try {
      // DISABLED: Now handled by Zoho CRM workflow "3 of 5 Value Check-in and Save Deals" (fires at 2 weeks).
      // To re-enable, remove the `if (false)` wrapper below.
      if (false) {
