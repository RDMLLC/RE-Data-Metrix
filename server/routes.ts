...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ============================================
  // SYNC SEED DATA ENDPOINTS (Individual by Type)
  // Uses ID-based matching for reliability
  // ============================================

  // Preview all sync changes without applying
  app.get("/api/admin/sync-preview", ensureAdmin, async (req, res) => {
    try {
      const existingCategories = await storage.getAllAffiliateCategories();
      const existingAffiliates = await storage.getAllAffiliates();
      const existingLenders = await storage.getAllLenders();
      const existingLoanProducts = await db.select().from(loanProducts);
      
      const existingCategoryIds = new Set(existingCategories.map(c => c.id));
      const existingAffiliateIds = new Set(existingAffiliates.map(a => a.id));
      const existingLenderIds = new Set(existingLenders.map(l => l.id));
      const existingProductIds = new Set(existingLoanProducts.map((p: { id: string }) => p.id));

      const preview = {
        affiliateCategories: {
          toAdd: seedAffiliateCategories.filter(c => !existingCategoryIds.has(c.id)).map(c => c.name),
          toUpdate: seedAffiliateCategories.filter(c => existingCategoryIds.has(c.id)).map(c => c.name),
          notInSeed: existingCategories.filter(c => !seedAffiliateCategories.find(s => s.id === c.id)).map(c => c.name)
        },
        affiliates: {
          toAdd: seedAffiliates.filter(a => !existingAffiliateIds.has(a.id)).map(a => ({ name: a.name, isActive: a.isActive })),
          toUpdate: seedAffiliates.filter(a => existingAffiliateIds.has(a.id)).map(a => {
            const existing = existingAffiliates.find(e => e.id === a.id);
            return { 
              name: a.name, 
              currentActive: existing?.isActive, 
              newActive: a.isActive,
              willChange: existing?.isActive !== a.isActive
            };
          }),
          notInSeed: existingAffiliates.filter(a => !seedAffiliates.find(s => s.id === a.id)).map(a => ({ name: a.name, isActive: a.isActive }))
        },
        lenders: {
          toAdd: seedLenders.filter(l => !existingLenderIds.has(l.id)).map(l => l.companyName),
          toUpdate: seedLenders.filter(l => existingLenderIds.has(l.id)).map(l => l.companyName),
          notInSeed: existingLenders.filter(l => !seedLenders.find(s => s.id === l.id)).map(l => l.companyName)
        },
        loanProducts: {
          toAdd: seedLoanProducts.filter((p: { id: string }) => !existingProductIds.has(p.id)).map((p: { productName: string }) => p.productName),
          toUpdate: seedLoanProducts.filter((p: { id: string }) => existingProductIds.has(p.id)).map((p: { productName: string }) => p.productName),
          notInSeed: existingLoanProducts.filter((p: { id: string }) => !seedLoanProducts.find(s => s.id === p.id)).map((p: { productName: string }) => p.productName)
        }
      };
      ...
      res.status(500).json({ error: "Failed to generate preview", details: String(error) });
    }
  });

  // Sync Affiliate Categories (by ID)
  app.post("/api/admin/sync-affiliate-categories", ensureAdmin, async (req, res) => {
    try {
      console.log('Syncing affiliate categories...');
      const existingCategories = await storage.getAllAffiliateCategories();
      ...
      for (const category of seedAffiliateCategories) {
        if (existingIds.has(category.id)) {
          await db.update(affiliateCategories)
            .set({ name: category.name, description: category.description, sortOrder: category.sortOrder })
            .where(eq(affiliateCategories.id, category.id));
          updated++;
        } else {
          await db.insert(affiliateCategories).values(category).onConflictDoNothing();
          added++;
        }
      }

      res.json({ success: true, message: `Categories synced: ${added} added, ${updated} updated`, results: { added, updated } });
    } catch (error) {
      console.error('Sync categories error:', error);
      res.status(500).json({ error: "Failed to sync categories", details: String(error) });
    }
  });

  // Sync Affiliates (by ID) - ONLY updates isActive field, preserves all other manual edits
  app.post("/api/admin/sync-affiliates", ensureAdmin, async (req, res) => {
    try {
      console.log('Syncing affiliates (isActive only)...');
      const existingAffiliates = await storage.getAllAffiliates();
      const existingIds = new Set(existingAffiliates.map(a => a.id));
      
      let added = 0, updated = 0, unchanged = 0;
      const changes: { name: string; from: boolean; to: boolean }[] = [];
      
      for (const affiliate of seedAffiliates) {
        if (existingIds.has(affiliate.id)) {
          const existing = existingAffiliates.find(a => a.id === affiliate.id)!;
          
          // Only update if isActive actually changed
          if (existing.isActive !== affiliate.isActive) {
            changes.push({ name: affiliate.name, from: existing.isActive, to: affiliate.isActive });
            
            // ONLY update isActive - preserve all other manually edited fields
          ...
          // Insert new affiliate with seed data
          await db.insert(affiliates).values({
            id: affiliate.id,
            name: affiliate.name,
            description: affiliate.description,
      ...
      console.log('Affiliate sync changes:', changes);
      res.json({ 
        success: true, 
        message: `Affiliates synced: ${updated} status changes, ${added} added, ${unchanged} unchanged`,
        results: { added, updated, unchanged, changes }
      });
    } catch (error) {
      console.error('Sync affiliates error:', error);
      res.status(500).json({ error: "Failed to sync affiliates", details: String(error) });
    }
  });

  // Sync Lenders (by ID) - ONLY updates isPreferred status, preserves all other manual edits
  app.post("/api/admin/sync-lenders", ensureAdmin, async (req, res) => {
    try {
      console.log('Syncing lenders (isPreferred only)...');
      const existingLenders = await storage.getAllLenders();
      const existingIds = new Set(existingLenders.map(l => l.id));
      
      let updated = 0, unchanged = 0, notFound = 0;
      const changes: { name: string; from: boolean; to: boolean }[] = [];
      
      for (const lender of seedLenders) {
        if (existingIds.has(lender.id)) {
          const existing = existingLenders.find(l => l.id === lender.id)!;
          
          // Only update if isPreferred actually changed
          if (existing.isPreferred !== lender.isPreferred) {
            changes.push({ name: lender.companyName, from: existing.isPreferred, to: lender.isPreferred });
            
            // ONLY update isPreferred - preserve all other manually edited fields
            await db.update(lenders)
              .set({ isPreferred: lender.isPreferred })
              .where(eq(lenders.id, lender.id));
            updated++;
          } else {
            unchanged++;
          ...
          notFound++;
          console.log(`Lender "${lender.companyName}" not found in database (seed ID: ${lender.id})`);
        }
      }

      res.json({ 
        success: true, 
        message: `Lenders synced: ${updated} preference changes, ${unchanged} unchanged, ${notFound} not found`,
        results: { updated, unchanged, notFound, changes },
        note: "New lenders must be added through normal registration/invite flow"
      });
    } catch (error) {
      console.error('Sync lenders error:', error);
      res.status(500).json({ error: "Failed to sync lenders", details: String(error) });
    }
  });

  // Sync Loan Products (by ID) - ONLY updates isActive status, preserves all other manual edits
  app.post("/api/admin/sync-loan-products", ensureAdmin, async (req, res) => {
    try {
      console.log('Syncing loan products (isActive only)...');
      ...
      for (const product of seedLoanProducts) {
        if (existingIds.has(product.id)) {
          const existing = existingProducts.find((p: { id: string; isActive: boolean }) => p.id === product.id)!;
          
          // Only update if isActive actually changed
          if (existing.isActive !== product.isActive) {
            changes.push({ name: product.productName, from: existing.isActive, to: product.isActive });
            
            // ONLY update isActive - preserve all other manually edited fields
            await db.update(loanProducts)
              .set({ isActive: product.isActive })
              .where(eq(loanProducts.id, product.id));
            updated++;
          } else {
            unchanged++;
          }
          ...
          notFound++;
          console.log(`Loan product "${product.productName}" not found in database (seed ID: ${product.id})`);
        }
      }

      res.json({ 
        success: true, 
        message: `Loan products synced: ${updated} status changes, ${unchanged} unchanged, ${notFound} not found`,
        results: { updated, unchanged, notFound, changes },
        note: "New loan products must be added through lender portal"
      });
    } catch (error) {
      console.error('Sync loan products error:', error);
      res.status(500).json({ error: "Failed to sync loan products", details: String(error) });
    }
  });

  // Legacy endpoint - redirect to individual syncs
  app.post("/api/admin/sync-seed-data", ensureAdmin, async (req, res) => {
    res.status(400).json({ 
      error: "This endpoint has been deprecated. Please use the individual sync endpoints.", 
      endpoints: [
        "/api/admin/sync-affiliate-categories",
        "/api/admin/sync-affiliates", 
        "/api/admin/sync-lenders",
        "/api/admin/sync-loan-products"
      ]
    });
  });
